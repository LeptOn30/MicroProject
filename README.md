...
To run all services using docker-compose, navigate to the **.tools/docker** directory:
...
To stop all services, run the following command in the **.tools/docker** directory:

```
docker-compose stop
```

To run a specific service, run the following command in the **.tools/docker** directory:
...
The application connects to the MicroProject backend through the API Gateway running on `localhost:8080`. The backend consists of several microservices:
...
The application now includes a complete login system that authenticates users against the MicroProject backend and manages JWT tokens.
...
- Configured API endpoint: `http://localhost:8080/api/v1/auth/login`
- Method calls the AuthController in mproject-user-account-service
...
- Added prominent **MicroProject** branding with gradient logo
...
2. **API Call**: POST request to `http://localhost:8080/api/v1/auth/login`
...
- ✅ **MicroProject Branding**: Prominent logo with gradient styling and platform tagline
...
The application now includes a dedicated Projects page accessible through header navigation, displaying the complete project list from the mproject-project-service backend.
...
- ✅ **Real Project Data**: Fetches from mproject-project-service API
...
**Note**: Make sure the MicroProject backend services are running on `localhost:8080` (Gateway) for the frontend to work properly.
...
│                                                                         │
│    npm ci          →    ng build         →    dist/mproject-angular/    │
...
# Optionally you can run "podman compose up build" ...
docker-compose up --build

# Build and run (background)
# Optionally you can run "podman compose up build --no-cleanup" ...
docker-compose up -d --build   

# View logs
# Optionally you can run "podman compose logs -f" ...
docker-compose logs -f   

# Stop containers
# Optionally you can run "podman compose down" ...
docker-compose down   
...
```bash
# Build the image
# Optionally you can run "podman build -t mproject-angular ." ...
docker build -t mproject-angular .

# Run the container
# Optionally you can run "podman run -p 4200:80 mproject-angular" ...
docker run -p 4200:80 mproject-angular   

# Run in background
# Optionally you can run "podman run -d -p 4200:80 --name mproject-angular mproject-angular" ...
docker run -d -p 4200:80 --name mproject-angular mproject-angular   

# Stop and remove
# Optionally you can run "podman stop mproject-angular && podman rm mproject-angular" ...
docker stop mproject-angular && docker rm mproject-angular   
```
...
│ mproject-angular   │ ~30 MB      │ NGINX + built Angular app             │
└────────────────────┴─────────────┴───────────────────────────────────────┘
```
...
│   ┌─────────────────────────────────────────────────────────────────┐   │
│   │  Project name: MicroProject Angular                             │   │
│   │  Project ID:   mproject-angular (auto-generated, can edit)      │   │
│   └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
...
│                                                                         │
│   Project name:   MicroProject Angular                                  │
│   Project ID:     mproject-angular  ← Copy this value                   │
...
```
coverage/mproject-angular/index.html
```
...
The `@analogjs/vite-plugin-angular` (required for full Angular + Vitest integration) it's not yet fully supportted for Angular 20.
